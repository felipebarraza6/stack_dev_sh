...
const Dga = () => {
    ...
    const getDataSh= async() =>{
        const rq = await sh.get_data_send_dga(state.selected_profile.id).then((r)=>{
            let today = new Date(); 
          ...
          setData([{
            nivel: r.results[0].nivel ? parseFloat(state.selected_profile.d3-r.results[0].nivel).toFixed(1):0,
            caudal: r.results[0].flow ? r.results[0].flow:0,
            acumulado: r.results[0].total ? numberForMiles.format(r.results[0].total):0,
            fecha: `${r.results[0].date_time_medition.slice(0,10)} ${r.results[0].date_time_medition.slice(11,16)}`
          }])
