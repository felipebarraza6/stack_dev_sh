export async function getNovusData(setCaudal, setNivel, state, api_novus, setAcumulado, acumulado, nivel){
    ...
    const rqAcumulado = await api_novus.lastData(state.selected_profile.title=='Paine'?'wifiaccva':'3grecdi1va', state.selected_profile.token_service).then((x)=>setAcumulado(
