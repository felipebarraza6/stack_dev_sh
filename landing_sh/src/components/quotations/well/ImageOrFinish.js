...
const ImageOrFinish = () => {
    ...
    return(<>
        {state.wells.temporary_well.is_load_image ? 
            <ImageLoader />:<Row style={{paddingBottom:state.wells.list.length == 1 ? '17%': state.wells.list.length ==2 ? '10%':'0%'}}>
            
            <Col span={window.innerWidth > 800 ? 15:24} style={{paddingLeft:'10px'}}>
                {state.wells.list.length>0 &&
                    <Table dataSource={state.wells.list} title={()=><Title level={4}>Listado de pozos ingresados correctamente</Title>} 
                        columns={[
                            {
                            ...
                            {                             
                                render: (x)=><>
                                    <Button type='primary' onClick={()=>{
                                        ...
                                        dispatch({type:'SET_CURRENT', step:1})
