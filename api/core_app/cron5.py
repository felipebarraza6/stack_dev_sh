...
def get_novus_and_save_in_api():
    id_pf_filter = [55, 56, 57, 58]
    clients = ProfileClient.objects.filter(
        is_monitoring=True, id__in=id_pf_filter)
    ...
    for client in clients:
        ...
        for variable in client_serializer['variables']:
            ...
            if client_serializer['is_thethings']:
                parsed_url = (
                    f"https://api.thethings.io/v2/things/{token}/resources/{
                        variable['str_variable']}/?limit=1"
                )
                request = requests.get(parsed_url)
            else:
                parsed_url = (
                    f"https://api.tago.io/data/?variable={
                ...
                if client_serializer['is_thethings']:
                    if data and data[0].get("value"):
                        response["total"] = parser_total(
                            data[0].get("value"), client_serializer['scale'])
                    else:
                        response["total"] = "0"
                else:
                    if data and data.get("result"):
                        response["total"] = parser_total(
                            data.get("result")[0].get("value"), client_serializer['scale'])
                    else:
                        response["total"] = "0"
